param($AppName='D4TEST', $ServerInstance='localhost', $Port='6379', $dbUser='D4TestUser', $redisDbNum=4)
#Step 1 - Create Directory for new Installation
$AppPath = "C:\Creatio\$AppName"
...
$mdf = 'C:\SQLDB\'+$dbFile.BaseName+'.mdf'
$ldf = 'C:\SQLDB\'+$dbFile.BaseName+'.ldf'
...
Begin`
	...
	select @SqlStatement = 'CREATE LOGIN [$dbUser] WITH PASSWORD=N'+'''D4TestUser'''+', DEFAULT_DATABASE=[master], CHECK_EXPIRATION=OFF, CHECK_POLICY=OFF'`
	EXEC sp_executesql @SqlStatement`
End
"
...
$con = Get-Content $cs
$myHost = iex 'hostname'
$con | % { $_.Replace("host=TSAGENT-2-6;db=0;port=6379", "host=$myHost;db=4;port=6379;maxReadPoolSize=10;maxWritePoolSize=500") } | Set-Content $cs
